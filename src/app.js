...
import { getErrorExit } from "./utils/getError.js";
...
export const run = async () => {
    await pipeline(
        input
            ? fs.createReadStream(input).on('error', () => getErrorExit('Ошибка открытия файла'))
            : process.stdin,
        new CaesarCipherTransform({action, shift, oldOutput}),
        output
            ? fs.createWriteStream(output).on('error', () => getErrorExit('Ошибка записи файла'))
            : process.stdout,
    )
}
